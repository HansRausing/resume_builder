...
export function bulletsTextToFlowCvUlHtml(bulletsText) {
  ...
  const items = raw
    ...
    .map((s) => {
      let html = escapeFlowCvPlainText(s);
...
export function tailoredResumeJsonToFlowCvWorkSaveBodies(tailoredResumeJson) {
  ...
  const blocks = Array.isArray(json.workExperienceBulletsOnly) ? json.workExperienceBulletsOnly : [];
  ...
  for (let i = 0; i < FLOWCV_WORK_ENTRY_FIXED.length; i++) {
    ...
    bodies.push({
      ...
      entry: {
        ...fixed,
        description: hasContent ? bulletsTextToFlowCvUlHtml(bulletsText) : '<ul></ul>',
